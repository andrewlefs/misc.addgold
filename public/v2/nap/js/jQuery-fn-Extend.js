jQuery.fn.extend({
    ...
    customNotify: function (options) {
        ...
        var span = $("<span>").addClass("notify-cricle " + options.type);
        $("<img>").attr("src", "/v1/inapp/images/ic_check_black_24dp_1x.png").appendTo(span);
    ...
    //function 
    customLockScreen: function (options) {
        options = options == undefined ? {
            text: "Notify",
            type: "success"
        } : options;
        ...
        this.wrapper = $("<div>")
                .addClass("toast-popup mdl-shadow--4dp")
                .addClass(options.type)
                .appendTo(this);
        $("<div>").addClass("wrapper").appendTo(this.wrapper);
        this.panel = $("<div>").addClass("panel").appendTo(this.wrapper);
        $("<i>")
                .text("check")
                .addClass("material-icons mdl-shadow--2dp").appendTo(this.panel);
        $("<span>")
                .text(options.text)
                .appendTo(this.panel);
        var wrapper = this.wrapper;
    ...
    customLoading: function () {
        this.wrapper = $("<div>")
                .addClass("toast-popup mdl-shadow--4dp")
                .addClass("mdl-loading")
                .appendTo(this);
        $("<div>").addClass("wrapper").appendTo(this.wrapper);
        this.panel = $("<div>").addClass("loading").appendTo(this.wrapper);
        var divLoader = $("<div>").addClass("material-icon failed-loader").appendTo(this.panel);
        $("<span>").addClass("first").appendTo(divLoader);
        $("<span>").addClass("second").appendTo(divLoader);
        $("<span>").addClass("third").appendTo(divLoader);
        var wrapper = this.wrapper;
        //console.log(wrapper);
    },
    //function 
    customStopLoading: function () {
        $(this).children(".mdl-loading").remove();
