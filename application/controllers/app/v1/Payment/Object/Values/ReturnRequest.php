...
class ReturnRequest extends AbstractObject {
    ...
    public function export() {
        ...
        if ($this->message == NULL) {
            $oClass = new \ReflectionClass(__CLASS__);
            $oContants = $oClass->getConstants();
            if ($type = array_search($this->code, $oContants)) {
                $data["message"] = $type;
            }
        }
        return $data;
    ...
    public function OutOfJsonResponse() {
        ...
        $secretList = new SecretKeyList();
        $hashkey = $secretList->getSecretKey($this->app);
        $pathLog = new \Payment\Logger\PathLogger();
        ...
        $data = $this->export();        
        unset($data["app"]);
        $encryptData = Security::encrypt(json_encode($data), $hashkey);
        
        if (empty($path) === TRUE)
            return;
        try {
            $path = $path . "/payment/" . date($date);

            if (!file_exists($path))
                @mkdir($path, 0777, TRUE);
            $fp = fopen($path . "/". date("d") . ".txt" , "a");
            $text = "Request Time: \t" . date("Y-m-d H:i:s") . "\t" .  json_encode($data) . "\t" . $encryptData . "\r\n\r\n";
